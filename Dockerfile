...
RUN git -C /usr/local/src clone https://github.com/cybergate-services/Signal-Server.git
...
COPY --from=build /usr/local/src/Signal-Server/websocket-resources/target/websocket-resources-2.55.jar /usr/share/websocket-resources.jar
...
RUN chown -R signal /usr/share/websocket-resources.jar
USER signal
